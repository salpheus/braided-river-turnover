...
root = '/Volumes/SAF_Data/SAF_Data/remote-data/watermasks/C02_1987-2023_may'
...
# ms_path = '/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/full/maskstack'
# # riv = 'agubh2'
# for riv in rivlist[12:]:
#     print(riv)
...
#     np.save(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/holes_filled_polys/{riv}.npy', Ihf)

#     plt.figure(dpi = 300)
#     plt.imshow(Ihf, cmap = 'gray')
#     plt.title(riv)
#     plt.savefig(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/holes_filled_polys/{riv}_holesfilled.png')
#     plt.close()
# df of max wettable area 
# polys = glob.glob('/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/holes_filled_polys/*.npy')

# for poly in polys:
#     arr = np.load(poly)
#     print(np.sum(arr.ravel()))
    # print(np.count_nonzero(arr))
#%% create masks of bars only using hole filler 

### NOTE: Aug 15. hard drive crashed aug 8th; didn't rerun this because it didnt make proper bar masks, some of the annual polygons cut out smaller channels
...
#         plt.savefig(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/bar_masks_imgs/{riv}_{yr+1999}_barmask.png')
#         plt.close()
        
#     ## save binary numpy array of the holes filled polygon
#     np.save(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/bar_masks/{riv}_barmask.npy', bar_maps)

    
#%% actually calculate ndvi
...
for riv in rivlist:    
    ...
    max_boundary = np.load(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/holes_filled_polys/{riv}.npy')
    lat_crop, long_crop = np.where(max_boundary == 0) ## get indices of pixels to replace with 0
    
    ndvi_composite = np.zeros([25, max_boundary.shape[0], max_boundary.shape[1]])
    ndvi_composite_water = np.zeros([25, max_boundary.shape[0], max_boundary.shape[1]])
    ...
    for yr, img in enumerate(img_paths):
        ...
        ## only for ob_up and ob_down
        # cropped_im[abs(cropped_im)<0.003] = np.nan
        ndvi_composite_water[yr, :, :] = Ndvi(cropped_im) ## calculate NDVI only within the active channel belt, including water pixels
        ...
        # query = np.where(ndvi_composite[14, :, :]<-1)
        # ## ob check
        # plt.scatter(query[1], query[0], marker = '*')
        
        

        plt.savefig(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/ndvi_maps_qc/{riv}_{yr+1999}.png', dpi = 300)
        plt.close()
        
    np.save(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/ndvi_stacks_withwater/{riv}.npy', ndvi_composite_water)
    np.save(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/ndvi_stacks_withoutwater/{riv}.npy', ndvi_composite)
...
ndvi_df.to_csv('/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover/masters/ndvi_composite_stats.csv')

#%% make values outside of 1 to -1 nans

ndvi_df_clip = pd.DataFrame(index = rivlist)

ndvi_ars = glob.glob('/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/ndvi_stacks_withoutwater/*.npy')

for ar in ndvi_ars:
    # ar = ndvi_ars[16]
    nm = ar.split('/')[-1].split('.npy')[0]
    print(nm)
    arr = np.load(ar)
    arr[arr>1] = np.nan
    arr[arr<-1] = np.nan
    ...
    # plt.imshow(arr[-1, :, :])
    ndvi_df_clip.loc[nm, 'min'] = np.nanmin(arr)
    ndvi_df_clip.loc[nm, 'mean'] = np.nanmean(arr)
    ndvi_df_clip.loc[nm, 'med'] = np.nanmedian(arr)
    ndvi_df_clip.loc[nm, 'max'] = np.nanmax(arr)
    ndvi_df_clip.loc[nm, 'q1'] = np.nanquantile(arr, 0.25)
    ndvi_df_clip.loc[nm, 'q3'] = np.nanquantile(arr, 0.75)
    
    np.save(f'/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/ndvi_stacks_withoutwater_clipped/{nm}.npy', arr)

ndvi_df_clip.to_csv('/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover/masters/0_ndvi-nowater_composite_clip_stats.csv')
