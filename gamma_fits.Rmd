...
turn_freq_path = '/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover/masters/px_turnfreq_0s/numbers_only'
stats_path = '/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover'
fig_path = '/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover/masters/px_turnfreq_0s/dist_figs'
...
NEW 2025!! Fit the distributions to the distributions of numbers of turnovers per pixel in each river (across the entire corridor)
```{r}
## fit distributions to the ebi data
# Get a list of all CSV files in the folder
tf_csvs <- list.files(turn_freq_path, pattern = "\\.csv$", full.names = TRUE)

# Create a summary dataframe to store the results
turnfreq_df <- data.frame(
  File = character(),
  Shape = numeric(),
  Rate = numeric(),
  stringsAsFactors = FALSE
...
# Iterate through each CSV file
for (csv_file in tf_csvs) {
  # Read the CSV file into a dataframe
  data <- as.numeric(read.csv(csv_file, header=FALSE)[[1]])
  ...
  # Add the results to the summary dataframe
  turnfreq_df <- rbind(turnfreq_df, data.frame(
    File = basename(csv_file),
    Shape = shape,
    Rate = rate
      ))
  ...
  # Plot the fitted distribution
  # Optional: Save each plot as a PNG file
  png_filename <- file.path(fig_path, paste0("gamma_fit_", tools::file_path_sans_ext(basename(csv_file)), ".png"))
  png(filename = png_filename)
  plot(fit)
  dev.off()
}
    
summary_path = '/Volumes/SAF_Data/SAF_Data/remote-data/arrays/C02_1987-2023_allLS_db/1999_nc_turnover/masters/px_turnfreq_0s'

# Optional: Save the summary dataframe to a CSV file
write.csv(turnfreq_df, file.path(summary_path, "pxfreq_0s_gam_mme_sum.csv"), row.names = FALSE)
